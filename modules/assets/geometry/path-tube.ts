...
export function parallelTransportFrames (points: THREE.Vector3[]): TransportFrames {
  if (points.length < 2)
    throw new Error('parallelTransportFrames: need at least 2 points')
  ...
  for (let i = 0; i < size; i++) {
    const prev    = points[Math.max(0, i - 1)] as THREE.Vector3
    const next    = points[Math.min(size - 1, i + 1)] as THREE.Vector3
    const tangent = new THREE.Vector3().subVectors(next, prev)
    if (tangent.lengthSq() < 1e-12)
      tangent.copy(tangents.at(-1) ?? new THREE.Vector3(0, 1, 0))
  ...
  for (let i = 1; i < size; i++) {
    const tPrev = tangents[i - 1] as THREE.Vector3
    const tCurr = tangents[i] as THREE.Vector3
    const nCurr = (normals[i - 1] as THREE.Vector3).clone()
    const bCurr = (binormals[i - 1] as THREE.Vector3).clone()
...
export function createPathTube (
  points: THREE.Vector3[],
  { radius = 1, radialSegments = 12, inward = false, vRepeat = 1 }: PathTubeOptions = {},
  ...
  for (let i = 0; i <= tubularSegments; i++) {
    ...
    const r = Math.max(0, finiteRadius(radiusAt(t, i)))

...
export function createConnectionGraph (
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    for (const { j } of distances.slice(0, Math.max(0, Math.round(neighbors)))) {
  ...
  return {
    ...
    edges,
    setProgress (t) {
      uniforms.uProgress.value = THREE.MathUtils.clamp(t, 0, 1)
    },
    setHighlight (nodeIndex) {
      uniforms.uHighlight.value = nodeIndex ?? -1
    },
    dispose () {
      if (disposed)
        return

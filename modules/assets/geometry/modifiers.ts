...
import { hash3 } from '../../../lib/state/rng.js'
...
function axisExtent (geo: THREE.BufferGeometry, axis: Axis): AxisExtentReturnType {
...
export function applyTwist (geo: THREE.BufferGeometry, angle: number, axis: Axis = 'y'): THREE.BufferGeometry {
...
export function applyTaper (geo: THREE.BufferGeometry, factor: number, axis: Axis = 'y'): THREE.BufferGeometry {
  const pos = geo.getAttribute('position') as THREE.BufferAttribute | undefined
  if (!pos || pos.count === 0)
    return geo
...
export function applyBend (geo: THREE.BufferGeometry, angle: number, axis: Axis = 'x'): THREE.BufferGeometry {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const bendInto: Axis = axis === 'y' ? 'z' : 'y'
...
export function displaceByNoise (geo: THREE.BufferGeometry, options: NoiseDisplaceOptions = {}): THREE.BufferGeometry {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    pos.setXYZ(
      i,
      scratch.x + nor.getX(i) * d,
      scratch.y + nor.getY(i) * d,
      scratch.z + nor.getZ(i) * d,
    )
  }
  pos.needsUpdate = true
  geo.computeVertexNormals()
...
export function simplifyGeometry (geo: THREE.BufferGeometry, targetCount: number): THREE.BufferGeometry {
  const position = geo.getAttribute('position')
  if (!position || position.count < 4 || targetCount <= 0)
    return geo.clone()
  return new SimplifyModifier().modify(geo, Math.min(Math.round(targetCount), position.count - 3))
}

/** Subdivide long edges. `maxEdgeLength` in world units, `iterations` passes. */
export function tessellateGeometry (geo: THREE.BufferGeometry, maxEdgeLength = 0.1, iterations = 6): THREE.BufferGeometry {
  if (!geo.getAttribute('position'))
    return geo.clone()
  return new TessellateModifier(Math.max(1e-6, maxEdgeLength), Math.max(0, Math.round(iterations))).modify(geo)
}

/** Split shared vertices across hard edges so flat-shaded creases stay sharp. */
export function edgeSplit (geo: THREE.BufferGeometry, cutOffAngleRad = Math.PI / 6, keepNormals = false): THREE.BufferGeometry {
  if (!geo.getAttribute('position'))
    return geo.clone()
...
export function mergeVertices (geo: THREE.BufferGeometry, tolerance = 1e-4): THREE.BufferGeometry {
  if (!geo.getAttribute('position'))
    return geo.clone()
  return BufferGeometryUtils.mergeVertices(geo, Math.max(Number.EPSILON, tolerance))
}

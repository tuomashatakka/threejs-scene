...
function positive (value: number, fallback = 1e-6): number {
  return Number.isFinite(value) ? Math.max(fallback, Math.abs(value)) : fallback
}
...
export function roundedRectShape (width: number, height: number, radius: number): THREE.Shape {
  ...
  const r  = Math.max(0, Math.min(Number.isFinite(radius) ? radius : 0, width / 2, height / 2))
  const hw = width / 2
  const hh = height / 2
...
export function polygonShape (sides: number, radius: number): THREE.Shape {
  sides = Math.max(3, Math.round(Number.isFinite(sides) ? sides : 3))
...
export function starShape (points: number, outerRadius: number, innerRadius: number): THREE.Shape {
  points = Math.max(2, Math.round(Number.isFinite(points) ? points : 2))
  outerRadius = positive(outerRadius)
  innerRadius = Math.min(outerRadius, positive(innerRadius))
...
export function gearShape (teeth: number, outerRadius: number, innerRadius: number, toothDepth = 0.25): THREE.Shape {
  teeth = Math.max(3, Math.round(Number.isFinite(teeth) ? teeth : 3))
  outerRadius = positive(outerRadius)
  innerRadius = Math.min(outerRadius * 0.95, positive(innerRadius))
  toothDepth = THREE.MathUtils.clamp(Number.isFinite(toothDepth) ? toothDepth : 0.25, 0, 0.95)
...
export function ringShape (outerRadius: number, innerRadius: number): THREE.Shape {
  outerRadius = positive(outerRadius)
  innerRadius = Math.min(outerRadius * 0.999, Math.max(0, Number.isFinite(innerRadius) ? innerRadius : 0))

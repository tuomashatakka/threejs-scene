...
export class BurnInPass extends Pass {
  ...
  private blendQuad = new FullScreenQuad(BLEND_SHADER.clone())
  private copyQuad = new FullScreenQuad(COPY_SHADER.clone())
  ...
  setDecay (decay: number): void {
    (this.blendQuad.material as THREE.ShaderMaterial).uniforms.uDecay!.value = decay
  }
  ...
  override render (
    ...
    const blend                    = this.blendQuad.material as THREE.ShaderMaterial
    ...
    const copy                    = this.copyQuad.material as THREE.ShaderMaterial
  ...
  override dispose (): void {
    ...
    (this.blendQuad.material as THREE.ShaderMaterial).dispose();
...
export function createBurnInPass ({ decay = 0.92 }: BurnInOptions = {}): BurnInPass {

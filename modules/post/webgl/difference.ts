...
export interface DifferenceOptions {
  // The texture to diff the incoming scene against.
...
export function createDifference (options: DifferenceOptions = {}): DifferencePass {
  const { compare = null, scale = 1 } = options
  const pass                          = new ShaderPass(DIFFERENCE_SHADER) as DifferencePass

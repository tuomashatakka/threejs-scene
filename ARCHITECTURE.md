...
modules/                // behavior on top of the public API — every entry is a
  ...
  assets/               //   content layer: plain factories, not app modules
    ...
    materials.ts        //     PBR, toon and procedural matcap factories
    *-material.ts       //     holographic + triplanar shader materials
    textures.ts         //     five seeded, DOM-free procedural DataTextures
    ...
    kit.ts              //     16 wasteland props, each ONE merged geometry
    scatter.ts          //     placement solver (keep-out rules) + InstancedMesh
    geometry/           //     profiles, sweeps, modifiers, merge/layout/bounds,
                        //       graphs, path tubes, infinite ground, rounded rocks
    definition.ts       //     procedural PropDefinition contract
    registry.ts         //     explicit owned prop registries
    create.ts           //     createProp/tryCreateProp — one synchronous front door
    composite.ts        //     owned groups of props
    instanced.ts        //     one InstancedMesh per mesh part
    catalog.ts          //     executable presets + ASSET_MANIFEST/schema
    authoring/          //     model spec, forgiving validator, build/review/retry
  physics/              //   OPTIONAL layer over cannon-es (optional peer dep)
    world.ts            //     fixed-step world + offset-aware body/object sync
    cloth.ts            //     particle grid + distance constraints
    liquid.ts           //     spatial-hashed PBF + marching-cubes surface
...
site/                   // vite-built public site: landing (index.html), one
  ...
  templates/            //   runnable starter apps — each one a worked
...
particles and voxels. Each lands as an additive layer — nothing here blocks it.

LLM prop authoring now lives inside `modules/assets`: a JSON dialect small models
can actually hit, a validator that repairs their habitual mistakes rather than
rejecting them, a deterministic compiler to `Prop`, and a critic that measures
the built geometry so the model can be told what is wrong and try again. The
same module owns procedural definitions, registries, composites, instancing,
geometry/material/texture factories, `CREATE_PROP_SCHEMA`, and the serializable
`ASSET_MANIFEST`. Nothing in the spec/build pipeline touches the DOM or a GL
context, so it runs server-side and headless.
...
so it steps the same headless as in a browser) in a fixed-step AppModule. Rigid
bindings retain the visual-origin/collider-centre offset, kinematics sync before
steps, and `addEach()` splits direct children. Cloth uses cannon particles and
constraints; liquid uses cannon spheres for rigid contacts plus a spatial-hashed
position-based density solver and a marching-cubes surface. Nothing else imports
cannon-es; a consumer who never touches physics never installs it.
